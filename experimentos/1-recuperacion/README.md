...
Documentos y resultados de la **Tabla 5.1 de la memoria** («Recuperación a nivel de
*chunk*. 615 preguntas por configuración, K = 10, generador gemma-4 26B-A4B»).
...
lleva la frase literal del documento que la responde y la sección que la contiene. Sobre
los mismos candidatos recuperados se comparan tres estrategias de ordenación y dos modelos
de *embedding*, con K = 10 y el generador fijo para aislar el efecto del recuperador. Un
fragmento cuenta como relevante si su sección contiene la respuesta, con umbral de
*recall* léxico 0,7.
...
| Ruta | Contenido |
|---|---|
| `preguntas/` | 22 conjuntos, uno por protocolo: la pregunta, la sección esperada y la cita literal del documento que la responde |
...
| `resultados/*/section_metrics_summary.json` | las métricas agregadas: **aquí están las cifras de la tabla** |
| `resultados/*/section_overrides.json` | las correcciones de etiquetado hechas tras revisión humana |

Cada `results_*.json` recoge, por pregunta: la consulta, la sección esperada, la cita de
...
2 de los 22 protocolos (PE27 y PA195). Afectan a algo más
del 5 % de los pares y su efecto es homogéneo entre estrategias, por lo que no altera la
ordenación entre ellas.

## Un matiz sobre el enunciado

El pie de la Tabla 5.1 dice «a nivel de *chunk*», pero las métricas publicadas son las
claves `sec@0.7_*`: **relevancia a nivel de sección**. Un fragmento cuenta como relevante
si la sección a la que pertenece contiene la respuesta. Es coherente con el diseño del
sistema, que reconstruye la sección completa a partir de cualquiera de sus fragmentos.
